...
/******** OWNER NUMBER**********/
const ownerNumber = ["6281333782061@s.whatsapp.net","6282233385336@s.whatsapp.net"] 
/************************************/
			...
			switch(command) {
				...
				case 'bcgc':
                   ...
                   if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					if (!isRegistered) return reply(ind.noregis())
					client.updatePresence(from, Presence.composing) 
					teks = `\`\`\`Ini adalah list group 𝙳𝚎𝚊 𝙱𝙾𝚃 :\n\n\`\`\``
					no = 0
            ...
            await client.sendMessage(from, `Pong!!!!\nSpeed: ${processTime(time, moment())} _Second_`)
               ...
               client.sendMessage(from, resul, text, { quoted: mek})
				...
				case 'info':
					...
					teks = `*𝙳𝚎𝚊 𝙱𝙾𝚃* : ${me.name}\n*OWNER* : *AMPIBI*\n*AUTHOR* : FAJAR\n*+62 813-3378-2061* : @${me.jid.split('@')[0]}\n*Prefix* : ${prefix}\n*Total Block Contact* : ${blocked.length}\n*The bot is active on* : ${kyun(uptime)}`
			...
			if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `*「 𝙳𝚎𝚊 𝙱𝙾𝚃 BROADCAST 」*\n\n${body.slice(4)}`)
						}
						reply('𝙨𝙪𝙘𝙘𝙚𝙨𝙨 𝙗𝙧𝙤𝙖𝙙𝙘𝙖𝙨𝙩 ')
					}
					break
