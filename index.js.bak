...
async function starts() {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const mod = [ownerNumber,"6281333782061@s.whatsapp.net","6282250221294@s.whatsapp.net","6283842133528@s.whatsapp.net","6285933658859@s.whatsapp.net","6281266216466@s.whatsapp.net","6282233385336@s.whastapp.net"]
			const adminbotnumber = ["6281333782061@s.whatsapp.net"]
			const frendsowner = ["6281333782061@s.whatsapp.net"]
			const premium = ["6281333782061@s.whatsapp.net","6282250221294@s.whatsapp.net","6283842133528@s.whatsapp.net","6285933658859@s.whatsapp.net","6281266216466@s.whatsapp.net"]
			...
			switch(command) {
				...
				if (!isPublic) return reply(mess.only.public)
					...
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					client.sendMessage(from, '*𝙼𝚎𝚐𝚞𝚖𝚒𝚗 𝙱𝙾𝚃 GROUP*\n\n_Yuk masuk kak_\n_sama sama belajar buat bot_\n*https://chat.whatsapp.com/EVajiprT7BQ24ulX5Tefxm*', text, { quoted: mek })
					break
					case 'funmenu':
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
			       ...
			       case 'valxlove':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT 𝙼𝚎𝚐𝚞𝚖𝚒𝚗 ✨* 」\n*│+wa.me/6281333782061*\n*│+wa.me/6282250221294*\n*╰──────*「 *𝙼𝚎𝚐𝚞𝚖𝚒𝚗* 」*────*\n\n*_JIKA INGIN MENJADI ADMIN  𝙼𝚎𝚐𝚞𝚖𝚒𝚗 BOT_*\n*_Ketik /iklan_*' })
					break
					case 'premiumcek':
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					sa = await getBuffer(`https://i.ibb.co/Gv5W97v/IMG-20210106-WA1995.jpg`)
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*╭────*「 *PREMIUM USER👑* 」\n*│+ wa.me/6281333782061*\n*│+ wa.me/6282250221294*\n*│+ wa.me/6285933658859*\n*│+ wa.me/6281266216466*\n*│+ wa.me/6283842133528*\n*╰──────*「 * 𝙼𝚎𝚐𝚞𝚖𝚒𝚗* 」*────*\n\n*_JIKA INGIN MENJADI PREMIUM USER  𝙼𝚎𝚐𝚞𝚖𝚒𝚗 𝙱𝙾𝚃_*\n*_Ketik /daftarvip*' })
					break
					case 'cekmod': 
...
if (isBanned) return reply(mess.only.benned)    
 ...
 if (isBanned) return reply(mess.only.benned)    
          ...
          reply('Wait..')
				...
				if (!isUser) return reply(mess.only.userB)
              if (!isUser) return reply(mess.only.userB)
                  if (args.length < 1) return reply(`Masukan Teks\nContoh : ${prefix} 𝙼𝚎𝚐𝚞𝚖𝚒𝚗 𝙱𝙾𝚃`)
                data = await getBuffer(`https://docs-jojo.herokuapp.com/api/blackpink?text=${bodyXPTN.slice(7)}`)
...
if (isBanned) return reply(mess.only.benned)    
 ...
 if (isBanned) return reply(mess.only.benned)    
			...
			if (!isPublic) return reply(mess.only.public)
					...
					mentions('╔══✪〘 Mention All 〙✪══'+teks+'╚═〘  𝙼𝚎𝚐𝚞𝚖𝚒𝚗 𝙱𝙾𝚃 〙', members_id, true)
					break
				case 'clearall':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
